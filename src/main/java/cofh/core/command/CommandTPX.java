...
public class CommandTPX implements ISubCommand {
	...
	public void handleCommand(ICommandSender sender, String[] arguments) {
			...
			try {
				...
				if (!player.equals(playerSender)) {
					...
					if (playerSender.dimension == player.dimension) {
						...
						CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.otherToSelf",
						...
						player.setPositionAndUpdate(playerSender.posX, playerSender.posY, playerSender.posZ);
						CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.dimensionOtherToSelf",
							player.getCommandSenderName(),
				...
				CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.dimensionSelf",
					playerSender.worldObj.provider.getDimensionName(),
				...
				if (!player.equals(otherPlayer)) {
					...
					if (otherPlayer.dimension == player.dimension) {
						...
						CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.otherTo", player.getCommandSenderName(),
							otherPlayer.getCommandSenderName(),
							player.posX, player.posY, player.posZ);
					} else {
						EntityHelper.transferPlayerToDimension(player, otherPlayer.dimension,
							otherPlayer.mcServer.getConfigurationManager());
						player.setPositionAndUpdate(otherPlayer.posX, otherPlayer.posY, otherPlayer.posZ);
						CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.dimensionOtherTo",
							player.getCommandSenderName(),
							otherPlayer.getCommandSenderName(),
							player.worldObj.provider.getDimensionName(),
							player.posX, player.posY, player.posZ);
		...
		case 4: // (tpx <x> <y> <z>) teleporting self within dimension
			...
			CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.self",
				playerSender.posX, playerSender.posY, playerSender.posZ);
			break;
		case 5: // (tpx {<player> <x> <y> <z> | <x> <y> <z> <dimension>}) teleporting player within player's dimension orself to dimension
			try {
				...
				CommandHandler.logAdminCommand(sender, this, "info.cofh.command.tpx.other", player.getCommandSenderName(),
					player.posX, player.posY, player.posZ);
			} catch (PlayerNotFoundException t) {
				int dimension;
				try {
