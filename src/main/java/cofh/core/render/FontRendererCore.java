...
public class FontRendererCore extends FontRenderer {
	...
	public float renderUnicodeChar(char ch, boolean italic) {
		...
		ICustomCharRenderer iCustomCharRenderer = renderOverrides.get(ch);
		if (iCustomCharRenderer != null) {
			float v = iCustomCharRenderer.renderChar(ch, italic, posX, posY, this);
			if (v != -2) {
				return v;
			}
		}
