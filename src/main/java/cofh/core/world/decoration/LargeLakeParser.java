...
import cofh.lib.world.WorldGenLargeAdvLakes;
...
public class LargeLakeParser implements IGeneratorParser {

	@Override
	public WorldGenerator parseGenerator(String generatorName,
			JsonObject genObject, Logger log,
			List<WeightedRandomBlock> resList, int clusterSize,
			List<WeightedRandomBlock> matList) {
		boolean useMaterial = false;
		useMaterial = genObject.has("useMaterial") ? genObject.get(
				"useMaterial").getAsBoolean() : useMaterial;
		WorldGenLargeAdvLakes r = new WorldGenLargeAdvLakes(resList, useMaterial ? matList : null);

		if (genObject.has("outlineWithStone")) {
			r.outlineBlock = genObject.get("outlineWithStone").getAsBoolean() ? Arrays
					.asList(new WeightedRandomBlock(Blocks.stone, 0)) : null;
		}
		ArrayList<WeightedRandomBlock> list = new ArrayList<WeightedRandomBlock>();
		if (genObject.has("outlineBlock")) {
			if (!FeatureParser.parseResList(genObject.get("outlineBlock"),
					list, true)) {
		...
		if (genObject.has("gapBlock")) {
			if (!FeatureParser.parseResList(genObject.get("gapBlock"), list,
					true)) {
