...
public class FeatureParser {
	...
	public static Block parseBlockName(String blockRaw) {
		...
		int i = 0;
		return Block.REGISTRY.getObject(new ResourceLocation(blockTokens.length == 1 ? "minecraft" : blockTokens[i++], blockTokens[i]));
	}
	...
	public static WeightedRandomItemStack parseWeightedRandomItem(JsonElement genElement) {
		...
		if (genElement.isJsonPrimitive()) {
			stack = new ItemStack(Item.REGISTRY.getObject(new ResourceLocation(genElement.getAsString())), 1, metadata);
		} else {
			...
			if (item.has("oreName") && ItemHelper.oreNameExists(item.get("oreName").getAsString())) {
				...
				stack = new ItemStack(Item.REGISTRY.getObject(new ResourceLocation(item.get("name").getAsString())), stackSize, metadata);
			}
			if (item.has("nbt")) {
				try {
