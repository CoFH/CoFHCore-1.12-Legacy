...
/**
 * The core of the CodeChickenLib render system. Rendering operations are written to avoid object allocations by reusing static variables.
 */
public class CCRenderState {
	...
	public static interface IVertexOperation {
		...
		/**
		 * Load any required references and add dependencies to the pipeline based on the current model (may be null) Return false if this operation is
		 * redundant in the pipeline with the given model
		 */
		public boolean load();
		...
		/**
		 * Get the unique id representing this type of operation. Duplicate operation IDs within the pipeline may have unexpected results. ID shoulld be
		 * obtained from CCRenderState.registerOperation() and stored in a static variable
		 */
	...
	public static interface IVertexSource {
		 ...
		 * @param <T>
		 *            The attrute array type
		 * @return An array, or null if not computed
		 */
		public <T> T getAttributes(VertexAttribute<T> attr);

		/**
		 * @return True if the specified attrute is provided by this model, either by returning an array from getAttributes or by setting the state in
		 *         prepareVertex
		 */
